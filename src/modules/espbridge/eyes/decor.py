...
def zen(d, W, H, now, ox=0.0, oy=0.0):  # cherry blossoms ride a gusting breeze down from the top-left -- "in flow"
    petal = ((0.0, 1.05), (-0.5, 0.4), (-0.68, -0.3), (-0.5, -0.82), (-0.22, -0.98),  # notched sakura petal:
             (0.0, -0.46), (0.22, -0.98), (0.5, -0.82), (0.68, -0.3), (0.5, 0.4))     # pointed stem, V-notch at the tip
    G0 = 3.0                                                                        # steady breeze (px/s), blowing right
    WIND = ((4.0, 0.20, 0.0), (2.6, 0.55, 1.3), (1.3, 1.20, 4.0))                    # layered gusts: (amp px/s, omega, phase)
    windv = G0 + sum(a * math.sin(w * now + p) for a, w, p in WIND)                  # live wind -> wilder flutter in gusts
    for i in range(5):                                              # at most 5 petals on screen
        T = 8.0 + i % 3 * 2.0                                       # this petal's spawn interval (s)
        ph = now / T + i * 0.37
        lap = int(ph)
        r = lambda n: math.sin((i * 9 + lap * 131 + n * 7.1) * 12.9898) * 43758.5453 % 1.0  # random, re-rolled each fall
        m = 0.15 + r(0) * 0.85                                      # this petal's WEIGHT: 0 light .. 1 heavy -- drives all of it
        fall = 0.45 + m * 0.45                                      # heavy falls through more quickly (less of the cycle)
        frac = ph - lap
        if frac > fall:
            continue                                                # gone -> idle until it spawns again (count ebbs 2..5)
        ft = frac / fall                                           # 0..1 down the screen
        elapsed = frac * T                                         # seconds aloft
        swp = elapsed * (2.2 + (1 - m) * 3.6) + r(3) * 6           # falling-leaf flutter phase (light = faster)
        sway = math.sin(swp) * (1.3 + (1 - m) * 3.0) * (0.7 + max(0.0, windv) * 0.12)  # side-to-side, wilder in gusts
        # horizontal = the integral of the wind this petal has actually felt since spawn (heavy = weaker pull + lag)
        windage, lag = 0.35 + (1 - m) * 1.9, m * 0.7
        te, ts = now - lag, now - lag - elapsed                    # 'felt' wind times: now and at spawn
        drift = G0 * elapsed
        for a, w, p in WIND:
            drift += a * (math.cos(w * ts + p) - math.cos(w * te + p)) / w
        x = 5 + r(1) * W * 0.18 + windage * drift + sway
        y = -6 + ft * (H + 14) + (1 - m) * 1.6 * math.sin(swp * 0.5)   # fall + a faint flutter bob
        ang = elapsed * (0.4 + (1 - m) * 1.2) * (1 if r(5) > 0.5 else -1) \
            + (0.35 + (1 - m) * 0.7) * math.cos(swp) + r(6) * 6    # slow spin + rock, 90deg out of phase with the sway
        fore = 0.45 + 0.55 * abs(math.cos(elapsed * (0.8 + (1 - m) * 1.2) + r(7) * 6))  # pinch thin when edge-on -> 3D flash
        wid, lng, bend = (2.6 + r(8) * 2.0) * fore, 3.0 + r(2) * 3.2, (r(5) - 0.5) * 0.9  # size + aspect (slim..round) + comma bow
